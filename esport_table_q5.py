...
mapped_watching = watching.apply(lambda x: pd.Series.map(x, values_map))
# mapped_statement = mapped_statement.rename_axis("answer").reset_index()
mapped_watching = mapped_watching.describe().T
df = mapped_watching.rename_axis("Odpowiedź").reset_index()
